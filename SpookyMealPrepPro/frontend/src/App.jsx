...
export default function App(){
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-10 flex items-center justify-center gap-3 p-4 bg-gradient-to-b from-white/90 to-transparent backdrop-blur">
        <div className="w-10 h-10 rounded-full grid place-items-center text-2xl shadow-xl" style={{background:'var(--primary)', color:'#ffffff'}}>🥗</div>
        <div>
          <div className="title text-2xl leading-5">Meal Prep — Pro</div>
          <div className="text-xs opacity-80">Nutrition-focused AI recipes</div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl p-4 grid gap-4">
        <section className="glass rounded-2xl p-4 grid gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <button className="px-3 py-2 rounded-xl border badge text-gray-700" onClick={()=>setUnitSystem(unitSystem==='metric'?'imperial':'metric')}>
              Units: <b className="ml-1">{unitSystem}</b>
            </button>
            <div className="flex gap-2">
              <input className="px-3 py-2 rounded-xl bg-white border border-gray-300" placeholder={unitSystem==='metric'?'Height (cm)':'Height (in)'} value={height} onChange={e=>setHeight(e.target.value)} />
              <input className="px-3 py-2 rounded-xl bg-white border border-gray-300" placeholder={unitSystem==='metric'?'Weight (kg)':'Weight (lb)'} value={weight} onChange={e=>setWeight(e.target.value)} />
            </div>
            <select className="px-3 py-2 rounded-xl bg-white border border-gray-300" value={goal} onChange={e=>setGoal(e.target.value)}>
              <option value="lose">Lose</option>
              <option value="maintain">Maintain</option>
              <option value="gain">Gain</option>
            </select>
            <select className="px-3 py-2 rounded-xl bg-white border border-gray-300" value={activity} onChange={e=>setActivity(e.target.value)}>
          ...
          <div className="glass rounded-2xl p-4 grid gap-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <select className="w-full px-3 py-2 rounded-xl bg-white border border-gray-300" value={spice} onChange={e=>setSpice(e.target.value)}>
                ...
                <div className="flex flex-wrap gap-2">
                  {['stovetop','oven','microwave','airfryer','slowcooker'].map(a => (
                    <button key={a} onClick={()=> setAppliances(p=> p.includes(a)? p.filter(x=>x!==a): [...p,a])}
                      className={"px-3 py-2 rounded-xl border " + (appliances.includes(a) ? "bg-green-100 border-green-400" : "badge")}>
        ...
        {meals.length>0 && (
          <section className="glass rounded-2xl p-4 grid gap-4">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {meals.map((m,i)=> (
                <div key={i} className="rounded-2xl p-4 border border-gray-300 bg-gradient-to-b from-white to-gray-50">
                  ...
                  <div className="font-black text-lg text-green-700">{m.spookyName}</div>
                  <div className="opacity-80 text-sm mb-2">{m.title}</div>
                  <div className="text-sm mb-2"><b className="text-accent" style={{color:'var(--accent)'}}>{m.calories}</b> kcal</div>
                  <div className="flex flex-wrap gap-2 text-xs mb-3">
                    <span className="badge px-2 py-1 rounded-full" style={{background:'rgba(129,199,132,0.15)', borderColor:'var(--primary)'}}>Protein {m.macros.protein_g}g</span>
                    <span className="badge px-2 py-1 rounded-full" style={{background:'rgba(255,112,67,0.15)', borderColor:'var(--accent)'}}>Carbs {m.macros.carbs_g}g</span>
                    <span className="badge px-2 py-1 rounded-full" style={{background:'rgba(129,199,132,0.15)', borderColor:'var(--primary)'}}>Fat {m.macros.fat_g}g</span>
                  </div>
            ...
            <div className="text-sm opacity-80">
              Healthy meals tailored to your nutritional goals
            </div>
          </section>
        )}
      </main>
      <footer className="text-center py-6 opacity-80 text-sm">Made with 🥗 for healthy living</footer>
    </div>
  )
}
